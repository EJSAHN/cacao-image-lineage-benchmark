#!/usr/bin/env python3
"""Shared utilities for matched-removal and visual-shortcut analyses."""
...
BASE_SEED = 20260801
REPEAT_SEEDS = [20260731 + 1009 * i for i in range(10)]

TASK_LABEL_ORDERS: dict[str, list[str]] = {
    "cacao_coarse_three_class": ["healthy", "black_pod", "frosty_pod"],
    "cocoamonilia_four_stage": ["healthy", "m1_hump", "m2_spot", "m3_sporulation"],
}
...
FEATURE_SETS = [
    "normal_efficientnet",
    "blur32_efficientnet",
    "grayscale_efficientnet",
    "border_only_efficientnet",
    "center_only_efficientnet",
    "color_histogram",
    "metadata",
]

VISUAL_FEATURE_TASKS = {
    1: "blur32_efficientnet",
    2: "grayscale_efficientnet",
    3: "border_only_efficientnet",
    4: "center_only_efficientnet",
    5: "color_histogram",
}
...
def normalize_text(series: pd.Series, missing: str = "missing") -> pd.Series:
    return series.fillna("").astype(str).str.strip().replace("", missing).str.lower()
...
def align_probabilities(raw: np.ndarray, model_classes: Sequence[str], labels: Sequence[str]) -> np.ndarray:
...
def aggregate_predictions_by_unit(
    frame: pd.DataFrame,
    labels: Sequence[str],
    unit_column: str = "final_strict_lineage_id",
) -> pd.DataFrame:
    ...
    for column in [
        ...
        "repeat_index", "split_seed", "heldout_source", "y_true", "final_strict_lineage_id",
        "final_split_block_id", "source_dataset_id",
    ]:
...
class ProjectPaths:
    ...
    @property
    def stage3b(self) -> Path:
        return self.root / "results" / "stage3b"

    @property
    def prepared(self) -> Path:
        return self.root / "results" / "stage3c_prepared"

    @property
    def feature_dir(self) -> Path:
        return self.big / "visual_features" / "stage3c"

    @property
    def feature_meta(self) -> Path:
        return self.root / "results" / "stage3c_features"

    @property
    def null_tasks(self) -> Path:
        return self.root / "results" / "stage3c_null_tasks"

    @property
    def shortcut_tasks(self) -> Path:
        return self.root / "results" / "stage3c_shortcut_tasks"

    @property
    def aggregate(self) -> Path:
        return self.root / "results" / "stage3c"

    @property
    def figures(self) -> Path:
        return self.root / "results" / "figures" / "stage3c"
